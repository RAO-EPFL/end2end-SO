...
class NetWind(nn.Module):
    def __init__(self, infeatures, relu=True):
        super().__init__()
        ...
        self.outlayer = nn.ReLU() if relu else nn.Softplus()
        self.cplayer = build_prescriptionlayer()

    def forward(self, x):
        return self.cplayer(self.outlayer(self.linear_relu_stack(x)).flatten())[0]
...
def build_prescriptionlayer():
    ...
    edlayer = CvxpyLayer(problem, parameters=[r], variables=[g])
    return edlayer
...
def test_myopic(df, relu):
    for shift in [1,3,6]:
        ...
        net = NetWind(3, relu).to(device)
        method = 'relu' if relu else 'softplus'
        run_myopic(test, loaders, length, net, f'results/e2e/opl_{method}/myopic', shift)
        
def test_myopic_stupid(df, relu):
    for shift in [1,3,6]:
        ...
        net = NetWind(2, relu).to(device)
        method = 'relu' if relu else 'softplus'
        run_myopic(test, loaders, length, net, f'results/e2e/opl_{method}/myopic_stupid', shift)


def test_historical(df, relu):
    SHIFTS = [3,6] if relu else [1,3,6]
    for shift in SHIFTS:
        ...
        net = NetWind(11, relu).to(device)
        method = 'relu' if relu else 'softplus'
        run_myopic(test, loaders, length, net, f'results/e2e/opl_{method}/historical', shift)
...
if __name__ == '__main__':
    ...
    test_myopic(df, True)
    test_myopic(df, False)
    test_myopic_stupid(df, False)
    test_myopic_stupid(df, True)
    test_historical(df, True)
    test_historical(df, False)
