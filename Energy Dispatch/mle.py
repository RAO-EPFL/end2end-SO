...
class NetWind(nn.Module):
    ...
    def prescript(self, x):
        return self.cplayer(nn.functional.relu(self.linear_relu_stack(x)).flatten())[0]
...
def run_myopic(testdata, loaders, length, net, resultpath, shift):
    ...
    with torch.no_grad():
        res = net.prescript(torch.from_numpy(testdata.drop('ActivePower', axis=1).to_numpy()).float().to(device))
...
def test_myopic(df):
    for shift in [1,3,6]:
        ...
        run_myopic(test, loaders, length, net, 'results/mle/myopic', shift)
...
def test_myopic_stupid(df):
    for shift in [1,3,6]:
        ...
        run_myopic(test, loaders, length, net, 'results/mle/myopic_stupid', shift)
...
def test_historical(df):
    for shift in [1,3,6]:
        ...
        run_myopic(test, loaders, length, net, 'results/mle/historical', shift)
